...
- (void)open:(int)quality {
    ...
    if (codecOpenedTimes++ != 0) {
        ...
        encodeState = speex_encoder_init(&speex_nb_mode);  // *****modify
        decodeState = speex_decoder_init(&speex_nb_mode);  // *****modify
...
- (NSData *)encode:(short *)pcmBuffer length:(int)lengthOfShorts {
	...
	for (int sampleIndex = 0; sampleIndex < nSamples; sampleIndex++) {
        ...
        nbBytes = speex_bits_write_whole_bytes(&encodeSpeexBits, cbits, encodeFrameSize);
